"""End-to-end tests: run zulip_build_report.py as CI does, plus build_report/context.py."""
...
from build_report.context import context_from_env
...
def test_mathlib_end_to_end(mathlib_log, tmp_path):
    ...
    assert "## linter.style.docStringVerso (3 warnings)" in text
    assert (
        "| Mathlib/Tactic/CrossRefAttribute.lean:217 | expected '_' without preceding space |"
    ) in text
...
def test_info_empty_means_false_like_the_shell():
    # The shell script used `${INFO:-false}`, so an empty INFO meant "no info".
    assert not context_from_env({"INFO": ""}).show_info


def test_stdout_is_utf8_regardless_of_locale(cslib_log, tmp_path):
    # Force a non-UTF-8 stdout encoding; the emoji headline must still be written.
    proc, _ = _run(cslib_log, {"REPO": "a/b", "SHA": "s", "RUN_ID": "1", "GITHUB_WORKFLOW": "W", "PYTHONIOENCODING": "latin-1"}, tmp_path)
    assert proc.stdout.startswith("zulip-message<<")
    assert "❌ W run on" in proc.stdout


def test_summary_budget_accounts_for_existing_content(mathlib_log, tmp_path):
    from build_report.summary import SUMMARY_LIMIT

    # Leave less room than the ~1.2 KB summary the fixture renders, so rows must be cut.
    existing = "x" * (SUMMARY_LIMIT - 600) + "\n"
    (tmp_path / "summary.md").write_text(existing, encoding="utf-8")
    _, summary = _run(mathlib_log, {"REPO": "a/b", "SHA": "s", "RUN_ID": "1", "INFO": "true", "GITHUB_WORKFLOW": "W"}, tmp_path)
    text = summary.read_text(encoding="utf-8")
    assert text.startswith(existing)
    assert len(text.encode("utf-8")) <= SUMMARY_LIMIT + 1_000  # only the fixed header may overshoot
