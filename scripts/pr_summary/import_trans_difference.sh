...
IFS=$'\n\t'
 ...
 : <<'BASH_MODULE_DOCS'
`scripts/pr_summary/import_trans_difference.sh <opt all> <opt_commit1> <opt_commit2>` outputs a full diff
...
getTransImports () {
  python3 "${SCRIPTS_DIR}/count-trans-deps.py" Mathlib |
...
git switch --detach "${currCommit}"

git switch --detach "${commit2}"
