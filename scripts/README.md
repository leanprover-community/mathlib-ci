...
- `declsDiff.py` takes two pre-computed `decls.txt` files (from
  ...
  it also emits the section heading and `<details>` wrap, for splicing into the comment.
- `updateDeclsDiffSection.py` PATCHes the `#### Declarations diff` section of a PR's
  `### PR summary` comment with the Lean-aware diff, by replacing the region between the
  `<!-- DECLS_DIFF_BEGIN/END -->` markers. Invoked post-build by `mathlib4`'s `decls-diff.yml`.
